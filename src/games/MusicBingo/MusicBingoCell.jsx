...
export default function MusicBingoCell({ song, artist, year, isMarked, onToggle }) {
  return (
    ...
    >
      ...
      <span className="text-[15px] sm:text-xl font-semibold leading-tight break-words line-clamp-3">
        {song}
      </span>
      <span className="text-xs sm:text-base font-normal opacity-80 leading-tight line-clamp-2">
        {artist}
      </span>
      {year != null && (
        <span className="hidden sm:inline text-sm font-normal opacity-60">
          {year}
        </span>
      )}
    </button>
